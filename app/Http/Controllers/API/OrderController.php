...
/*$cobon= Cobon::where(array('cobon_code' => $request->cobon_code,
'active'=>0))->first();
if(!empty($cobon)){

$cobons= Order::where(array('cobon_code' => $request->cobon_code,
'paid'=>'true'))->get();
if(count($cobons) >= $cobon->number_use){
    return $this->apiResponse4(false, ' Cobons Not Avaliable', 400);
}else{
    $cap=round($request->card_price -( $request->card_price * ($cobon->discount_percentage/100)),1);
}
   ...
   return $this->apiResponse4(false, ' Cobons Not Avaliable', 400);
    ...
    $cap= $this->checkcobon2($request->client_id,$request->cobon_code,$request->card_id);
}else{
    $cap=$request->card_price;
}


                        $request_data['card_id'] = $card->id;
                        $request_data['client_id'] = $request->client_id;
                        $request_data['card_price'] = $cap;
...
//echo floor($diff / 604800);
 ...
 $pacorder=  Packages_orders::where(array('client_id'=>$request->client_id,'active'=>0,'paid'=>'true'))->first(); 
...
$card= Cards::where(array('id' => $request->card_id))->first();

if(!empty($cobon)){

$cobons= Order::where(array('cobon_code' => $request->cobon_code,
'paid'=>'true','client_id'=>$request->client_id))->get();
...
if(in_array($request->card_id, $competition_all))
{
   ...
   return $this->apiResponse4(false, ' Cobons Not Avaliable', 200);
      ...
      public function checkcobon2($client_id,$cobon_code,$card_id)
    {
