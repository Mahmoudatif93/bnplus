...
class ProductsReport extends Controller
{
      public function index(Request $request)
    {
        ...
        if(isset($request->date1) && isset($request->date2) && !empty($request->searc_id) ){
        
           $date1=$request->date1;
            $date2=$request->date2;
            $searc_id=$request->searc_id;
            if($searc_id=='paid'){
                        $allprofits=NewProductsOrder::where('updated_at','>=' ,$date1)->where('updated_at','<=' ,$date2)->where('delevery',null)->where('paid','true')->with('ClientAddress')->orderBy('id', 'DESC')->get();
         ...
         $allprofits=NewProductsOrder::where('updated_at','>=' ,$date1)->where('updated_at','<=' ,$date2)->where('delevery',"yes")->where('paid','true')->with('ClientAddress')->orderBy('id', 'DESC')->get();
                         ...
                         $allprofits=NewProductsOrder::where('updated_at','>=' ,$date1)->where('updated_at','<=' ,$date2)->where('delevery',null)->where('paid','false')->with('ClientAddress')->orderBy('id', 'DESC')->get();
...
$allprofits=NewProductsOrder::with('ClientAddress')->orderBy('id', 'DESC')->get();
        ...
        return view('dashboard.ProductsReport.index', compact('allprofits','date1','date2'));
