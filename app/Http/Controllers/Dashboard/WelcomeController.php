...
$update1 = DB::table('profits')->where('id', 2)->update( [ 'profits' => $profit ]); 


if(isset($request->profitmanual)){
    $allmanual=DB::select('select * from profits');
    $mm=$allmanual[0]->manual +$request->profitmanual;
    $update2 = DB::table('profits')->where('id', 2)->update( [ 'manual' => $mm ]); 
    
     return redirect()->back();
}

$allprog=DB::select('select * from profits');
$pro=$allprog[0]->profits + $allprog[0]->manual;

    
        return view('dashboard.welcome', compact('profit','companies','orders','cards','clients','dubiorders','alldata','pro'));
